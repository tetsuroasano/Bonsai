...
float UnitMass = 2.333680475e+9; // in Msun
...
//    printf("#N -1\n");
//    printf("#id\ttype\tmass\tx\ty\tz\tvx\tvy\tvz\n");
...
// 
		...
		//bulge 1 disk 2
		if (reduceS == 2) UnitMass = 2334433308.;
		else if (reduceS == 1) UnitMass = 2328597746.;
    if (reduceS > 0)
    {
      typedef float float4[4];
      typedef float float3[3];
      ...
      for (size_t i = 0; i < n; i++)
      {
				if (idt->operator[](i).getType() != reduceS) continue;
        printf("%g\t%g\t%g\t%g\t%g\t%g\t%g\n",
                pos->operator[](i)[0],
                pos->operator[](i)[1],
                pos->operator[](i)[2],
