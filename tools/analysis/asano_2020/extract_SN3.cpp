...
// count SN canditates and calculate their mean Z and mean VZ
  ...
  {
		float r = SQ(pos->operator[](z)[0]);
		r  += SQ(pos->operator[](z)[1]);
		r = sqrt(r);
		if (r < rmin-maxDistanceTest || r > rmax+maxDistanceTest) continue;
		count_SN++;
...
void output_SN_snaps(const int myRank, std::string filename, float maxDistanceTest, float dSun, float angleToUse, int count_SN){
	...
	sprintf(outfile1, "./%s_SN_%g_%g_%g.csv", filename_only.c_str(), maxDistanceTest, dSun, angleToUse);
	if(myRank==0){
...
int main(int argc, char *argv[]){
    ...
    fprintf(stderr,"Usage: prog inFile dist angl(bar angle in rad) rmin rmax\n");
    ...
    float rmin = 5;
    rmin  = atof(argv[4]);
    fprintf(stderr,"Using the following R_min value: %f \n", rmin);
    
    float rmax = 10;
    rmax  = atof(argv[5]);
    fprintf(stderr,"Using the following R_max value: %f \n", rmax);
...
//READ////////////////////////////////////////////////////////////////////
	int n = read(myRank, comm, dataDM, in, reduceDM, idt, pos, vel, true, rmin, rmax, maxDistanceTest);
	in.close();
...
//Analysis////////////////////////////////////////////////////////////////
 ...
 float angle_list[72];
 for (int i = 0; i < 72; i++) {
	 angle_list[i] = -180 + 5*i;
 }
 int count_SN;
 for (int ii = 0; ii < (rmax-rmin)*2+5; ii++){
	 float dSun = rmin + 0.5*ii;
	 if (dSun > rmax) break;
	 for (int i = 0; i < 72; i++) {
		 float angleToUse = angle_list[i];
		 fprintf(stderr, "(R, phi)=(%f, %f)\n", dSun, angleToUse);
		 float angl2 = angleToUse*M_PI/180.;
		 rotate(angl2, n);
		 count_SN = Analysis(nRank, myRank, comm, n, dSun, maxDistanceTest);
...
{ 
  ...
  // stars
  Pbody_SN = new StructPbody[Nstars];
  int n_counts = 0;
