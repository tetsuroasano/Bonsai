...
void calc_surface_density(const int rank, const MPI_Comm &comm, float Omega[][N_phi_max], float Omega_sum[][N_phi_max], int N_particle){
  ...
  float	delta_phi = 2.0*M_PI/N_phi;
	...
	for (int i = 0; i < N_particle; i++) {
			...
			phi = acos(Pbody[i].Pos[0]/R); 
			if(Pbody[i].Pos[1]<0) phi = -phi + 2.0*M_PI;
...
void fourier_decomposition(int mode, float Omega[][N_phi_max], float phase[], float Amp[]){
	float phi, delta_phi = 2.0*M_PI/N_phi;
	float Ampcos,Ampsin;
	...
	for (int i = 0; i < N_R; i++) {
		...
		for (int j = 0; j < N_phi; j++) {
			Ampcos += cos(mode*phi+0.5*delta_phi)*Omega[i][j]*delta_phi;
			Ampsin += sin(mode*phi+0.5*delta_phi)*Omega[i][j]*delta_phi;
			phi += delta_phi;
		}
		Ampcos /= 2.0*M_PI;
		Ampsin /= 2.0*M_PI;
		...
		/*
		int label_max = 0;
		float phase_by_maximum;
		for (int j = 0; j < N_phi; j++) {
			if(Omega[i][j]>=Omega[i][label_max]) label_max = j;	
		}
		phase_by_maximum = delta_phi*label_max + 0.5*delta_phi;
		if(phase_by_maximum > M_PI) phase_by_maximum -= M_PI;
		phase[i] = phase_by_maximum;
		*/
		angle_a = atan(Ampsin/Ampcos)/mode + M_PI;
		...
		Amp[i] = Ampcos*Ampcos + Ampsin*Ampsin;
...
int main(int argc, char *argv[]){
	...
	if(myRank==0){
		ofs << "#Snap" << "\t" << "ave2" << "\t" << "ave3" << "\t" << "ave4" << "\t" << "ave5" << "\t" << "angle_best\n";
	}
	...
	while(ifs && getline(ifs, filename))
	{
		...
		if(myRank==0){
			for (int i = 0; i < N_R; i++) {
				cout << "R<" << unit_length*i <<": " << phase[i] << endl;
			}
		}
		...
		if(myRank==0){
			ofs << filename << "\t" << ave2 << "\t" << ave3 << "\t" << ave4 << "\t" << ave5 << "\t" <<  bar_angle_best(phase) << "\t" << endl;
			/*
				 for (int i = 0; i < N_R; i++) {
				 cout << i << " " << phase[i] << endl;
				 }
			...
			//	cout << 2.0*M_PI*i/N_phi << " " << Omega_sum[0][i] << endl;
			//}
	}
