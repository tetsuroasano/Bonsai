...
// count SN canditates and calculate their mean Z and mean VZ
  ...
  {
		if( ((SQ(pos->operator[](z)[0])+SQ(pos->operator[](z)[1]))>SQ(dSun-maxDistanceTest)) 
				&& ((SQ(pos->operator[](z)[0])+SQ(pos->operator[](z)[1]))<SQ(dSun+maxDistanceTest)) ){
    	count_SN++;
...
int main(int argc, char *argv[]){
    ...
    fprintf(stderr,"Usage: prog inFile dist angl(bar angle in rad) dSun\n");
...
//READ////////////////////////////////////////////////////////////////////
	int n = read(myRank, comm, dataDM, in, reduceDM, idt, pos, vel, true, dSun, maxDistanceTest);
	in.close();
...
//Analysis////////////////////////////////////////////////////////////////
 ...
 for (int i = 0; i < 72; i++) {
	 ...
	 cout << "Solar angle: " << angleToUse << endl;
