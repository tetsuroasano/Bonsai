...
template<typename IO, size_t N>
static double writeDM(std::vector<real4>    &bodyPositions, 
		std::vector<real4>    &bodyVelocities, 
		std::vector<IDType>   &bodiesIDs, 
		IO &out,
		std::array<size_t,N> &count)
{
	...
	/* write pos */
	{
		BonsaiIO::DataType<real4> pos("DM:POS:real4",pCount);
		for (int i = 0; i< pCount; i++)
			pos[i] = bodyPositions[i];
		double t0 = MPI_Wtime();
		out.write(pos);
...
int main(int argc, char * argv[]) {
	...
	std::vector<real4>    DMbodyPositions;
	std::vector<real4>    DMbodyVelocities;
	std::vector<IDType>   DMbodyIDs;
	...
	if (reduceDM > 0)
	{
		...
		auto idt = BonsaiIO::DataType<IDType>("DM:IDType");
		auto pos = BonsaiIO::DataType<float4>("DM:POS:real4");
		auto vel = BonsaiIO::DataType<float3>("DM:VEL:float[3]"); 		
		...
		for (size_t i = 0; i < n; i++) 
		{
			...
			positions.y = pos[i][1];
			positions.z = pos[i][2];
			positions.w = pos[i][3] * reduceDM;
			velocity.x = vel[i][0];
			velocity.y = vel[i][1];
			velocity.z = vel[i][2];
			DMbodyIDs.push_back(idt[i]);
			DMbodyPositions.push_back(positions);
			DMbodyVelocities.push_back(velocity);
		}
	...
	// write
	{
		...
		std::fill(ntypeglb.begin(), ntypeglb.end(), 0);
		MPI_Barrier(comm);
		dtWrite += writeDM(DMbodyPositions, DMbodyVelocities, DMbodyIDs, out, ntypeglb);
