 ...
 *      Author: jbedorf
 *  Modified by tetsuroasano
 *  Select DM particles based on their type decoded from the IDType integers
 */

#include "tipsyIO.h"

uint32_t getType(const uint64_t x)
{
	return static_cast<uint32_t>(x >> 48);
}
